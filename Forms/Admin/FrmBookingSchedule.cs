...
public partial class FrmBookingSchedule:Form
{
    ...
    private void SetStatus(string status){var id=SelectedId();if(id==null)return;if(status=="Cancelled"&&UiMsg.Ask("Hủy lịch đặt đã chọn? Đơn đã nhận tiền sẽ không được phép hủy trực tiếp.", "Xác nhận")!=DialogResult.Yes)return;try{_booking.SetStatus(id.Value,status);LoadData();Toast.Success("Đã cập nhật trạng thái lịch.");}catch(Exception ex){UiMsg.Warn(ex.Message,"Cập nhật lịch");}}
}
