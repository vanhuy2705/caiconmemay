...
public partial class FrmEmployeeAccount : Form
{
    ...
    private void Save(){
        ...
        try{
            ...
            Db.Execute("UPDATE NhanVien SET HoTen=@n,SoDienThoai=NULLIF(@p,N''),Email=NULLIF(@e,N'') WHERE NhanVienID=@id",
        ...
        }catch(Exception ex){UiMsg.Error(ex.Message,"Cập nhật tài khoản");}
    }
    private void ChangePassword(){
        using var f=new Form{Text="Đổi mật khẩu",StartPosition=FormStartPosition.CenterParent,Size=new Size(450,300),FormBorderStyle=FormBorderStyle.FixedDialog,MaximizeBox=false,MinimizeBox=false,BackColor=Color.White,Padding=new Padding(20)};
        var card=new RoundedPanel{Dock=DockStyle.Fill,Padding=new Padding(20),Radius=14,BorderColor=AppTheme.Border};
        var oldP=new TextBox{Left=20,Top=20,Width=360,PlaceholderText="Mật khẩu hiện tại",UseSystemPasswordChar=true};
        var newP=new TextBox{Left=20,Top=70,Width=360,PlaceholderText="Mật khẩu mới",UseSystemPasswordChar=true};
        var confirm=new TextBox{Left=20,Top=120,Width=360,PlaceholderText="Nhập lại mật khẩu mới",UseSystemPasswordChar=true};
        var b=new RoundedButton{Left=230,Top=170,Width=150,Height=40,Text="Đổi mật khẩu"};
        AppTheme.StylePrimary(b);
        foreach(var c in new Control[]{oldP,newP,confirm})AppTheme.StyleInput(c);
        b.Click+=(_,__)=>{
            try{
        ...
        card.Controls.AddRange(new Control[]{oldP,newP,confirm,b});f.Controls.Add(card);AppTheme.Upgrade(f);f.ShowDialogFx(this);
    }
}
